process GENOMAD_ENDTOEND {
    ...
    output:
    tuple val(sample_id), path("*_aggregated_classification/*_aggregated_classification.tsv")    , emit: aggregated_classification
    tuple val(sample_id), path("*_annotate/*_taxonomy.tsv")                                      , emit: taxonomy
    tuple val(sample_id), path("*_find_proviruses/*_provirus.tsv")                               , emit: provirus
    tuple val(sample_id), path("*_score_calibration/*_compositions.tsv")                         , emit: compositions                , optional: true
    tuple val(sample_id), path("*_score_calibration/*_calibrated_aggregated_classification.tsv") , emit: calibrated_classification   , optional: true
    tuple val(sample_id), path("*_summary/*_plasmid.fna")                                        , emit: plasmid_fasta
    tuple val(sample_id), path("*_summary/*_plasmid_genes.tsv")                                  , emit: plasmid_genes
    tuple val(sample_id), path("*_summary/*_plasmid_proteins.faa")                               , emit: plasmid_proteins
    tuple val(sample_id), path("*_summary/*_plasmid_summary.tsv")                                , emit: plasmid_summary
    tuple val(sample_id), path("*_summary/*_virus.fna")                                          , emit: virus_fasta
    tuple val(sample_id), path("*_summary/*_virus_genes.tsv")                                    , emit: virus_genes
    tuple val(sample_id), path("*_summary/*_virus_proteins.faa")                                 , emit: virus_proteins
    tuple val(sample_id), path("*_summary/*_virus_summary.tsv")                                  , emit: virus_summary
